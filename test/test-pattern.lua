...
function tests.match_fn()
  ...
  --Expect('match[f[2,1],cond[f[$x,$y],y_=$x-1]]', 'true')
  --Expect('match[f[3,1],cond[f[$x,$y],y_=$x-1]]', 'false')
  --Expect('match[f[2,3],cond[f[$x,$y],y_=$x-1]]', 'false')
end
...
function tests.match_sum()
  Expect('d:==match_vars[a+b,x_+y_]; dict.get[d,x_]=a and dict.get[d,y_]=b', 'true')
  Expect('d:==match_vars[a+b,y_+x_]; dict.get[d,y_]=a and dict.get[d,x_]=b', 'true')
end
